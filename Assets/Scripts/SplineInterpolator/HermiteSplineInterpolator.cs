...
public enum HermiteeEndPointsMode { AUTO, AUTOCLOSED, EXPLICIT }
public enum HermiteeWrapMode { ONCE, LOOP }
public delegate void HermiteOnEndCallback();

public class HermiteSplineInterpolator : MonoBehaviour
{
	HermiteeEndPointsMode mHermiteEndPointsMode = HermiteeEndPointsMode.AUTO;
	...
	HermiteOnEndCallback mHermiteOnEndCallback;
	...
	public void StartInterpolation(HermiteOnEndCallback endCallback, bool bRotations, HermiteeWrapMode mode)
	{
		if (mState != "Reset")
			throw new System.Exception("First reset, add points and then call here");

		mState = mode == HermiteeWrapMode.ONCE ? "Once" : "Loop";
		mRotations = bRotations;
		mHermiteOnEndCallback = endCallback;
	...
	public void Reset()
	{
		...
		mCurrentTime = 0;
		mRotations = false;
		mHermiteEndPointsMode = HermiteeEndPointsMode.AUTO;
	}
	...
	void SetInput()
	{
		...
		if (mHermiteEndPointsMode == HermiteeEndPointsMode.AUTO)
		{
			mNodes.Insert(0, mNodes[0]);
			mNodes.Add(mNodes[mNodes.Count - 1]);
		}
		else if (mHermiteEndPointsMode == HermiteeEndPointsMode.EXPLICIT && (mNodes.Count < 4))
			throw new System.Exception("Invalid number of points");
	}
	...
	void SetExplicitMode()
	{
		...
		mHermiteEndPointsMode = HermiteeEndPointsMode.EXPLICIT;
	}
	...
	public void SetAutoCloseMode(float joiningPointTime)
	{
		...
		mHermiteEndPointsMode = HermiteeEndPointsMode.AUTOCLOSED;
		...
		if (mCurrentTime >= mNodes[mCurrentIdx + 1].Time)
		{
			...
			else
			{
				if (mState != "Loop")
				{
					...
					if (mHermiteOnEndCallback != null)
						mHermiteOnEndCallback();
				}
				else
				{
					mCurrentIdx = 1;
