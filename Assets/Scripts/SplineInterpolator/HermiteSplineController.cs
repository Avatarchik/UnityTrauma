...
public enum HermiteeOrientationMode { NODE = 0, TANGENT }
...
public class HermiteSplineController : MonoBehaviour
{
	...
	public HermiteeOrientationMode OrientationMode = HermiteeOrientationMode.NODE;
	...
	void SetupSplineInterpolator(SplineInterpolator interp, Transform[] trans)
	{
		...
		{
			if (OrientationMode == HermiteeOrientationMode.NODE)
			{
				interp.AddPoint(trans[c].position, trans[c].rotation, step * c, new Vector2(0, 1));
			}
			else if (OrientationMode == HermiteeOrientationMode.TANGENT)
			{
				Quaternion rot;
				if (c != trans.Length - 1)
