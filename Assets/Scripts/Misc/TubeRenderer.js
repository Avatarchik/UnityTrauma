...
var colors : Color[];
var tris : int[];
var lastVertices : int[];
var theseVertices : int[];

// this gets rid of garbage collection here
function Init( length : int, cross : int )
{
	if ( length == lastLength && cross == lastCross )
		return;
	lastLength = length;
	lastCross = cross;

	meshVertices = new Vector3[length*cross];
	uvs  = new Vector2[length*cross];
	colors = new Color[length*cross];
	tris = new int[length*cross*6];
	lastVertices = new int[cross];
	theseVertices = new int[cross];
}
...
//	renderer.enabled=true;
	...
	Init(vertices.length,crossSegments);
